...
RSpec.describe 'Flights', type: :request do
  before do
    %w[JPA GRU SSA REC CNF BSB GIG MVD].each do |iata|
      create(:airport, iata: iata)
    end
  end
  ...
  context 'search request' do
    ...
    context 'when flights are available' do
      ...
      let(:url) { build_url(departure_date, 'GRU') }
                  ...
                  connections: [
                    {
                      ...
                      destination_airport: response_request.last.dig(:legs, 0, :segments, 0, :destination,
                                                                     :displayCode),
                      flight_number: response_request.last.dig(:legs, 0, :segments, 0, :flightNumber).to_i,
                      name_airline: response_request.last.dig(:legs, 0, :segments, 0, :operatingCarrier, :name),
                    ...
                    },
                    {
                      ...
                      destination_airport: response_request.last.dig(:legs, 0, :segments, 1, :destination,
                                                                     :displayCode),
  ...
  def build_url(departure_date, destination_airport)
    URI(
      "#{ENV.fetch('URL_API')}/search-one-way?cabinClass=economy&departDate=" \
      "#{departure_date.strftime('%Y-%m-%d')}&fromEntityId=JPA&returnDate=&toEntityId=#{destination_airport}"
    )
  end
